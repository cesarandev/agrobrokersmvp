...
const NAV_ITEMS = ['Inicio', 'Nosotros', 'Productos', 'Servicios', 'Blog', 'FAQ', 'SAGRILAFT', 'Contacto'];
...
const NAV_ANCHOR = { Inicio: '#inicio', Nosotros: '#nosotros', Productos: '#productos', Servicios: '#servicios', Blog: '#blog', FAQ: '#faq', SAGRILAFT: '#sagrilaft', Contacto: '#contacto' };
/* Dedicated page file for each item. SAGRILAFT has no standalone page yet → it lives as a section on the landing. */
const NAV_PAGE = { Inicio: HOME_PAGE, Nosotros: 'Nosotros.html', Productos: 'Productos.html', Servicios: 'Servicios.html', Blog: 'Blog.html', FAQ: 'FAQ.html', SAGRILAFT: 'SAGRILAFT.html', Contacto: 'Contacto.html' };
/* data-page value that marks each dedicated page as the current page */
const NAV_PAGEKEY = { Inicio: 'home', Nosotros: 'nosotros', Productos: 'productos', Servicios: 'servicios', Blog: 'blog', FAQ: 'faq', SAGRILAFT: 'sagrilaft', Contacto: 'contacto' };
...
/* Ítems del menú que, incluso desde el inicio, llevan a su página completa
   (su sección en el inicio es solo un teaser que invita a la página real). */
const NAV_TO_PAGE_FROM_HOME = ['Servicios'];

const navHref = (it) => {
  // En el inicio: la mayoría de ítems hacen scroll a su sección…
  if (CURRENT_PAGE === 'home') {
    return NAV_TO_PAGE_FROM_HOME.includes(it) ? NAV_PAGE[it] : NAV_ANCHOR[it];
  }
    ...
    <header className={'header ' + (solid ? 'solid' : 'transparent')}>
      <div className="wrap">
        <a href={homeHref} className="header__logo" aria-label="AgroBrokers inicio">
          <img src={solid ? 'assets/logo.png' : 'assets/logo-white.png'} alt="AgroBrokers" />
        </a>
        <nav className="nav">
          {NAV_ITEMS.map((it) =>
    ...
    <section className="hero" id="inicio">
      ...
      <div className="wrap hero__inner">
        ...
        <div className="hero__visual" aria-hidden="true">
          <img src="assets/hero.png" alt="Portafolio de productos AgroBrokers" className="hero__visual-img" />
        </div>
      </div>
      <div className="hero__trust">
        <I.shield />
        <span><b>15 años</b> en el mercado · Cobertura nacional</span>
      </div>
